...
HANDLE lock;
HANDLE semaphore_a, semaphore_b, semaphore_c, semaphore_d, semaphore_e, semaphore_f, semaphore_g, semaphore_h, semaphore_i, semaphore_k, semaphore_m, semaphore_f_Sync, semaphore_g_Sync, semaphore_h_Sync;
...
DWORD WINAPI thread_f(LPVOID lpParam) {
    ...
    ReleaseSemaphore(semaphore_h_Sync, 1, NULL);
    for (int i = 0; i < 3; i++) {
        WaitForSingleObject(semaphore_h_Sync, INFINITE);
        WaitForSingleObject(lock, INFINITE);
        std::cout << 'f' << std::flush;
        ReleaseMutex(lock);
        computation();
        ReleaseSemaphore(semaphore_f_Sync, 1, NULL);
    }
    WaitForSingleObject(semaphore_h_Sync, INFINITE);
...
DWORD WINAPI thread_g(LPVOID lpParam) {
    ...
    WaitForSingleObject(semaphore_h, INFINITE);
    for (int i = 0; i < 3; i++) {
        WaitForSingleObject(semaphore_f_Sync, INFINITE);
        WaitForSingleObject(lock, INFINITE);
        std::cout << 'g' << std::flush;
        ReleaseMutex(lock);
        computation();
        ReleaseSemaphore(semaphore_g_Sync, 1, NULL);
    }
...
DWORD WINAPI thread_h(LPVOID lpParam) {
    ...
    WaitForSingleObject(semaphore_f, INFINITE);
    for (int i = 0; i < 3; i++) {
        WaitForSingleObject(semaphore_g_Sync, INFINITE);
        WaitForSingleObject(lock, INFINITE);
        std::cout << 'h' << std::flush;
        ReleaseMutex(lock);
        computation();
...
int lab3_init() {
    ...
    if (semaphore_m == NULL){
        std::cerr << "CreateSemaphore error: " << GetLastError() << '\n'; return 1;}
    semaphore_f_Sync = CreateSemaphore(NULL, 0, 11, NULL);
    if (semaphore_f_Sync == NULL){
        std::cerr << "CreateSemaphore error: " << GetLastError() << '\n'; return 1;}
    semaphore_g_Sync = CreateSemaphore(NULL, 0, 11, NULL);
    if (semaphore_g_Sync == NULL){
        std::cerr << "CreateSemaphore error: " << GetLastError() << '\n'; return 1;}
    semaphore_h_Sync = CreateSemaphore(NULL, 0, 11, NULL);
    if (semaphore_h_Sync == NULL){
        std::cerr << "CreateSemaphore error: " << GetLastError() << '\n'; return 1;}
